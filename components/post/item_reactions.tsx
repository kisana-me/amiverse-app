  ...
  return (
    <View style={styles.container}>
      <View style={styles.content}>
        <TouchableOpacity
          style={[styles.reactionButton, post.is_reacted && styles.reactedButton]}
        >
          <View style={styles.icon}>
            <Svg viewBox="0 0 100 100" width={16} height={16} fill="currentColor" color={post.is_reacted ? '#e0245e' : '#666'}>
          ...
          <ThemedText style={[styles.count, post.is_reacted && styles.reactedCount]}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.scroll}>
          {post.reactions?.map((reaction) => (
            <TouchableOpacity
              key={reaction.aid}
              style={[styles.emojiButton, reaction.reacted && styles.reactedButton]}
            >
              <ThemedText style={styles.emoji}>{reaction.name}</ThemedText>
              <ThemedText style={[styles.count, reaction.reacted && styles.reactedCount]}>
